...
class Autofocusing(nn.Module):
    ...
    def __init__(self, masks):
        ...
        self.motion_parameters = nn.ParameterDict(
            dict(
    ...
    def forward(self, ks_input):
        ...
        translations = torch.stack(
            [self.motion_parameters["x_shifts"], self.motion_parameters["y_shifts"]],
            dim=-1,
        )
        ...
        shift[:, 0] = shift[:, 0] + (
            -rotation_matrix[:, 0, 0] * translations[:, 0]
            - rotation_matrix[:, 0, 1] * translations[:, 1]
        )
        shift[:, 1] = shift[:, 0] + (
            -rotation_matrix[:, 1, 0] * translations[:, 0]
            - rotation_matrix[:, 1, 1] * translations[:, 1]
        )
        ...
        torch_affine[:, :, -1] = torch_affine[:, :, -1] / (
            (torch.tensor(images[0, 0, ...].shape).to(images.device) * 2.0) - 1
        )

        grid = F.affine_grid(
            torch_affine, (self.num_movements, 2, x, num_lines), align_corners=True
        ).to(images.device)

        image_2d = torch.view_as_complex(
            F.grid_sample(
                torch.view_as_real(images.squeeze(1)).permute(0, 3, 1, 2),
                grid.float(),
                mode="bicubic",
                align_corners=False,
            )
        ...
        kspace_out = (ks_input.squeeze() * (1 - self.masks.sum(0)).float()) + (
            FFT(image_2d) * self.masks.float()
        ).sum(0)
