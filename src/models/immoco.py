...
class IMMoCo(nn.Module):
    ...
    def forward(self):
        ...
        motion_images = torch.view_as_complex(F.grid_sample(
                torch.view_as_real(images).permute(0,3,1,2) , grids , mode='bilinear', align_corners=False, padding_mode='zeros').permute(0,2,3,1).contiguous())
...
def imcoco_motion_correction(kspace_corr, masks, iters=200, learning_rate=1e-2, lambda_ge=1e-2, debug=False):
    ...
    if debug:
        ...
        print(f'Kspace input: {kspace_input.abs().min().item():.4f}, {kspace_input.abs().max().item():.4f}')
    ...
    for j in range(iters):
        ...
        if debug:
            if j % summar_steps == 0 or j == 199:
                ...
                axs[1].imshow(IFFT(kspace_foward_model.detach().cpu()).abs(), cmap='gray')
