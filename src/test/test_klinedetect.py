...
for scenario in scenarioius:
    ...
    for idx_test in trange(batch_test):
        ...
        with torch.no_grad():
            ...
            k_space_test = torch.view_as_real(k_space_test.view(bsz,1,H,W)).squeeze(1).permute(0,3,1,2)
            ...
            metrics.append({'dice': dice_score, 'iou': iou, 'sensitivity': calassification_metrics['Sensistivity'], 'specificity': calassification_metrics['Specificity'], 'presission': calassification_metrics['Presission'], 'f1': calassification_metrics['F1']})
            
    metrics_all.append(metrics)

# save metrics
torch.save(metrics_all, 'results/KLineDetect_metrics.pth')
...
metrics_all = torch.load('results/KLineDetect_metrics.pth')
...
with open('results/KLineDetect_metrics.tex', 'w') as f:
    ...
    f.write('\\topline\n')
    f.write('Scenario & Dice & IoU & Sensitivity & Specificity & Precision & F1 \\\\ \n')
    f.write('\\midline\n')
    for i in range(len(scenarioius)):
        f.write(scenarioius[i] + ' & ' + f'{(means_all[i*6].item()):.3f} + {stds_all[i*6].item():.3f}' + ' & ' + f'{(means_all[i*6+1].item()):.3f} + {stds_all[i*6+1].item():.3f}' + ' & ' + f'{(means_all[i*6+2].item()):.3f} + {stds_all[i*6+2].item():.3f}' + ' & ' + f'{(means_all[i*6+3].item()):.3f} + {stds_all[i*6+3].item():.3f}' + ' & ' + f'{(means_all[i*6+4].item()):.3f} + {stds_all[i*6+4].item():.3f}' + ' & ' + f'{(means_all[i*6+5].item()):.3f} + {stds_all[i*6+5].item():.3f}' + '\\\\ \n')
        f.write('\\bottomline\n')
    f.write('\\end{tabular}\n')
    f.write('\\end{table}\n')
...
axs[0].violinplot([metrics_for_plot[1]], positions=[2], showmeans=True, showmedians=True)
...
axs[1].violinplot([metrics_for_plot[2]], positions=[1], showmeans=True, showmedians=True)
axs[1].violinplot([metrics_for_plot[3]], positions=[2], showmeans=True, showmedians=True)
axs[1].set_title('IoU', fontsize=20)
axs[1].set_xticks([1, 2])
axs[1].set_xticklabels(['Light', 'Heavy'], fontsize=20)
axs[1].set_yticks([0.95, 0.98, 1.0])
axs[1].tick_params(labelsize=20, top=False, bottom=True, left=False, right=False, labelleft=False)
                   
axs[2].violinplot([metrics_for_plot[4]], positions=[1], showmeans=True, showmedians=True)
axs[2].violinplot([metrics_for_plot[5]], positions=[2], showmeans=True, showmedians=True)
...
axs[2].tick_params(labelsize=20, top=False, bottom=True, left=False, right=False, labelleft=False)

plt.tight_layout(pad=.4, w_pad=0.5, h_pad=0.3)
# save figure
fig.savefig('results/KLineDetect_metrics.png', bbox_inches='tight', dpi=300)
plt.close(fig)
