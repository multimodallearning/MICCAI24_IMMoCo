...
for scenario in scenarioius:
    ...
    for idx_test in trange(batch_test):
        ...
        with torch.no_grad():
            mask = (net(torch.view_as_real(k_space_test).squeeze(1).permute(0,3,1,2)).sigmoid() > 0.5).float()
        ...
        optimizer = torch.optim.Adam(AUTOFOCUS.parameters(), lr=1.)
    ...
    fig.savefig('results/autofocus_' + scenario + '.png', bbox_inches='tight')
    plt.close(fig)
    
    metrics_all.append(metrics)
    
# save metrics
torch.save(metrics_all, 'results/autofocus_metrics.pth')
...
metrics_all = torch.load('results/autofocus_metrics.pth')
...
with open('results/autofocus_metrics.tex', 'w') as f:
...
# # save figure
# fig.savefig('../results/autofocus_metrics.png', bbox_inches='tight')
# plt.close(fig)
