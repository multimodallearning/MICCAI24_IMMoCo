...
class IMMoCo(nn.Module):
    ...
    def forward(self):
        ...
        motion_images = torch.view_as_complex(
            F.grid_sample(
                torch.view_as_real(images).permute(0, 3, 1, 2).contiguous(),
                grids,
                mode="bilinear",
                align_corners=False,
                padding_mode="zeros",
...
def imcoco_motion_correction(
    ...
    with ClearCache():
        ...
        for j in range(iters):
            ...
            loss_inr = F.mse_loss(
                torch.view_as_real(kspace_foward_model),
                torch.view_as_real(kspace_input),
        ...
        for idx_test, file_name in zip(range(batch_test), file_names):
            ...
            with torch.no_grad():
                ...
                masks = extract_movement_groups(
                    mask.squeeze().sum(0).div(mask.squeeze().shape[0]) > 0.2,
                    make_list=True,
                ).cuda()
            ...
            image_prior, motion_out = imcoco_motion_correction(
            ...
            patch_metrics = evaluate_patches(
                image_prior.detach().abs(), image_gt.squeeze(), bboxes[idx_test]
            )
