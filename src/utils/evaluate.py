...
import torch
from piq import haarpsi, psnr, ssim
...
def normalize(x: torch.Tensor) -> torch.Tensor:
    ...
    if x.shape[0] > 1:
        ...
        return (x - min_b.view(-1, 1, 1, 1)) / ((max_b - min_b).view(-1, 1, 1, 1) + 1e-24)
    else:
...
def calmetric2D(pred_recon, gt_recon):
    ...
    ssim_array = ssim(pred, gt, data_range=1.0, kernel_size=ssim_kernel, reduction='mean')
...
def calmetric3D(pred_recon, gt_recon):
    ...
    for i in range(batch):
        psnr_array[i], ssim_array[i], haar_psi[i], rmse_array[i] = calmetric2D(pred_recon[i].unsqueeze(0), gt_recon[i].unsqueeze(0))
    
    return psnr_array.mean(), ssim_array.mean(), haar_psi.mean(), rmse_array.mean()

# write a function to calculate the end metrics for the whole dataset with mean and std. Prints it and save it as a latex table in the results folder
# its does this either for 3D or 2D depending if B=1 or B>1
def calc_metrics(pred_recon, gt_recon, save_path, name):
    # check sizes -> (B, C, D, H, W )
    if not pred_recon.ndim == 5 or not gt_recon.ndim == 5:
        batch = pred_recon.shape[0]
        if batch == 1:
            psnr_array, ssim_array, haar_psi, rmse_array = calmetric2D(pred_recon, gt_recon)
            print(f'PSNR: {psnr_array} db, SSIM: {ssim_array*100}%, HaarPSI: {haar_psi}, RMSE: {rmse_array}')
            ...
            psnr_array, ssim_array, haar_psi, rmse_array = calmetric3D(pred_recon, gt_recon)
            print(f'PSNR: {psnr_array} db, SSIM: {ssim_array*100}%, HaarPSI: {haar_psi}, RMSE: {rmse_array}')
            with open(os.path.join(save_path, name + '.csv'), 'w', newline='') as file:
                writer = csv.writer(file)
                writer.writerow(["PSNR", "SSIM", "HaarPSI", "RMSE"])
        ...
        for i in range(batch):
            psnr_array[i], ssim_array[i], haar_psi[i], rmse_array[i] = calmetric2D(pred_recon[i].unsqueeze(0), gt_recon[i].unsqueeze(0))
        print(f'PSNR: {psnr_array.mean()} db, SSIM: {ssim_array.mean()*100}%, HaarPSI: {haar_psi.mean()}, RMSE: {rmse_array.mean()}')
        with open(os.path.join(save_path, name + '.csv'), 'w', newline='') as file:
            writer = csv.writer(file)
            writer.writerow(["PSNR", "SSIM", "HaarPSI", "RMSE"])
            writer.writerow([psnr_array.mean(), ssim_array.mean(), haar_psi.mean(), rmse_array.mean()])
        # save as latex table
        df = pd.read_csv(os.path.join(save_path, name + '.csv'))
        df = df.round(3)
        df.to_latex(os.path.join(save_path, name + '.tex'), index=False, escape=False)
...
def create_violin_plot(data,method_names, metric_name='SSIM', save_path='./', name='violin_plot'):
    # chech if data is numpy array and length of method_names is correct
    if not isinstance(data, np.ndarray):
        raise TypeError("data must be a numpy array")
    if not len(method_names) == data.shape[1]:
        raise ValueError("length of method_names must be equal to the number of methods in data")
...
def metrics_classification(y_pred: torch.Tensor, y_gt: torch.Tensor):
    ...
    metrics_dict = {}
    metrics_dict['Sensistivity'] = sensitivity_score(
        true_positives, y_gt).item()
    metrics_dict['Specificity'] = specificity_score(true_negative, y_gt).item()
    metrics_dict['Presission'] = presission_score(
        true_positives, false_positives).item()
    metrics_dict['F1'] = f1_score(presission_score(
        true_positives, false_positives), sensitivity_score(true_positives, y_gt)).item()
...
if __name__ == '__main__':
    ...
    # # test
    # pred = torch.ones(10, 1, 256, 256).to(torch.float32)
    # gt = torch.ones(10, 1, 256, 256).to(torch.float32)

    # psnr_array, ssim_array, haar_psi, rmse_array = calmetric2D(pred, gt)
    # print(f'PSNR: {psnr_array} db, SSIM: {ssim_array*100}%, HaarPSI: {haar_psi}, RMSE: {rmse_array}')
    
    # # test calc_metrics
    # calc_metrics(pred, gt, './', 'test')

    # create random data with numpy for violin plot
