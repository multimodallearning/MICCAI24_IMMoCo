...
for i in trange(EPX):
    ...
    for batch_idx in range(batch_train):
        img_train = (
            IFFT(k_space_train[shuffeled_idx[batch_idx]])
            .view(bsz, channels, H, W)
            .cuda()
        )
        ...
        psnr_train.append(
            psnr(normalize_image(motion_refined), normalize_image(img_gt))
        )
    ...
    with torch.no_grad():
        ...
        for trgt in range(bsz_val):
            kspace_motion_val[trgt], _, _, _ = motion_simulation2D(
                img_val[trgt].squeeze()
            )
        ...
        ssim_val = ssim(
            normalize_image(motion_refined_val),
            normalize_image(img_val_gt),
            data_range=1.0,
        )
        psnr_val = psnr(
            normalize_image(motion_refined_val), normalize_image(img_val_gt)
        )
        ...
        sim_diff = (
            ssim_val
            - ssim(
                normalize_image(img_motion_val),
                normalize_image(img_val_gt),
                data_range=1.0,
            )
        ).abs()
...
ssim_motion = ssim(
    normalize_image(img_motion_val), normalize_image(img_val_gt), data_range=1.0
)
...
ssim_corr = ssim(
    normalize_image(motion_refined_val), normalize_image(img_val_gt), data_range=1.0
)
